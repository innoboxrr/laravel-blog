...
namespace Innoboxrr\LaravelBlog\Tests\Package;
...
final class PackageBootsTest extends TestCase
{
    ...
    public function test_las_migraciones_corren(): void
    {
        ...
        try {
            ...
            // Una migracion escrita para MySQL —INFORMATION_SCHEMA, borrar una
            // columna con clave foranea— no corre en SQLite, y eso no es un
            // defecto del paquete. Se salta diciendo cual, en vez de dar un rojo
            // que solo se arreglaria estropeando la migracion de produccion.
            if (preg_match('/INFORMATION_SCHEMA|after drop column|foreign key definition/i', $e->getMessage())) {
                $this->markTestSkipped('Migracion propia de MySQL: ' . strtok($e->getMessage(), "\n"));
            }
